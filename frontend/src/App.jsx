...
//             {tab === "signup" && (
//               <Inp label="Full Name *" name="name" value={f.name} onChange={ch} placeholder="your name"/>
//             )}
...
//               </div>
//               <span className="text-[11px] bg-white/25 text-white font-bold uppercase tracking-widest px-3 py-1 rounded-full">
//                 Upload Photo · AI test your skin
//               </span>
//             </div>
//           </div>
...
//             </h2>
//             <p className="text-slate-500 text-sm leading-relaxed mb-5">
//               Upload a selfie and our AI test and analyse ✨
//             </p>
//             <ul className="flex flex-col gap-2 mb-6">
...
//         Both features run on <strong>Replicate · protiva-nutrition</strong> · Secure · Private
//       </p>
//     </div>
//   );
// }
...
// Exact file name use karein jo aapke folder mein hai
// Inhe aise likhein (Line 5119 onwards)
const BANNER_IMG_1 = "/Gemini_Generated_Image_tybf2utybf2utybf.png";
const BANNER_IMG_2 = "/Gemini_Generated_Image_ducff4ducff4ducf.png";

// Conscious Chemist product images (public CDN URLs)
const CC_IMGS = [
  "https://www.consciouschemist.com/cdn/shop/files/Vitamin-C-Anti-Pigmentation-Face-Serum_1.webp?v=1714463785&width=600",
  "https://www.consciouschemist.com/cdn/shop/files/Retinol-0.3-Anti-Ageing-Serum_1.webp?v=1714463781&width=600",
  "https://www.consciouschemist.com/cdn/shop/files/SPF-55-Matte-Fluid-Sunscreen_1.webp?v=1714463783&width=600",
];

// ════════════════════════════════════════════════════════════
//  BANNER SLIDER COMPONENT
//  — Slides 1,2 use uploaded images as full background
//  — Slides 3,4,5 use product images from Conscious Chemist
//  — Auto-advances every 5s, dots + arrows for navigation
// ════════════════════════════════════════════════════════════
const BANNER_SLIDES = [
  {
    img:   BANNER_IMG_1,
    imgAlt:"Understand Your Skin Better With AI",
    imgSide:"right",   // image on right side
    badge: "AI Skin Analysis",
    title: "Understand Your Skin\nBetter With AI",
    text:  "Get a Free Skin Consultation Worth ₹1000",
    cta:   "Try Now",
    ctaAction: "Skin Test",
    ctaStyle: "blue",
    bg:    "bg-[#f0eeeb]",
  ...
  {
    img:   BANNER_IMG_2,
    imgAlt:"Glow From Within",
    imgSide:"right",
    badge: "Prottiva Nutrition",
    title: "Glow From Within",
    text:  "Healthy skin starts with the right balance of skincare and nutrition. Build a routine that works for your body and your lifestyle.",
    cta:   "Shop Now",
    ctaAction: "Shop",
    ctaStyle: "emerald",
    bg:    "bg-[#b8d4e8]",
  ...
  {
    img:   CC_IMGS[0],
    imgAlt:"Vitamin C Serum",
    imgSide:"right",
    badge: "Science-Backed",
    title: "Simple. Effective. Natural.",
    text:  "We combine science-backed skincare with proper nutrition to help you achieve clear skin, strong hair, and better overall health.",
  ...
  {
    img:   CC_IMGS[1],
    imgAlt:"Retinol Serum",
    imgSide:"right",
    badge: "Personalised Care",
    title: "Your Skin, Your Care",
    text:  "Everyone\'s skin is different. Get personalised solutions designed to improve your skin from the inside out.",
    cta:   "Take Skin Test",
    ctaAction: "Skin Test",
    ctaStyle: "emerald",
  ...
  },
  {
    ...
    imgAlt:"Sunscreen",
    imgSide:"right",
    badge: "Beauty + Nutrition",
    title: "Healthy Skin,\nStrong Hair",
    text:  "Support your body with the right nutrients and care routine to naturally improve your skin and hair strength.",
    cta:   "Shop Products",
    ctaAction: "Shop",
    ctaStyle: "emerald",
...
function BannerSlider({ setPage }) {
  ...
  // Auto-advance
  useEffect(() => {
    timerRef.current = setInterval(() => goTo((c) => (c + 1) % BANNER_SLIDES.length, "right"), 5000);
    return () => clearInterval(timerRef.current);
  }, []);

  function goTo(nextOrFn, dir) {
    if (animating) return;
    clearInterval(timerRef.current);
    const next = typeof nextOrFn === "function" ? nextOrFn(current) : nextOrFn;
    setAnimDir(dir || (next > current ? "right" : "left"));
    setAnimating(true);
    setTimeout(() => {
      setCurrent(next);
      setAnimating(false);
  ...
  function prev() { goTo((c) => (c - 1 + BANNER_SLIDES.length) % BANNER_SLIDES.length, "left"); }
  function next() { goTo((c) => (c + 1) % BANNER_SLIDES.length, "right"); }

  const s = BANNER_SLIDES[current];
  const isUploadedBg = current < 2;  // slides 0,1 use uploaded images as full bg

  return (
    <div className="relative w-full overflow-hidden select-none" style={{minHeight:"240px",maxHeight:"420px",height:"min(420px,55vw)"}}>

      {/* ── Slide ── */}
      <div
        key={current}
        className={`absolute inset-0 flex items-center ${s.bg} transition-all`}
        style={{
          opacity: animating ? 0 : 1,
          transform: animating
            ? `translateX(${animDir==="right"?"-3%":"3%"})`
            : "translateX(0)",
          transition: "opacity .35s ease, transform .35s ease",
        }}
      >
        {/* Full-bleed background image for uploaded slides */}
        {isUploadedBg && (
          <img
            src={s.img}
            alt={s.imgAlt}
        ...
        {/* Overlay for readability on uploaded images */}
        {isUploadedBg && (
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 flex items-center h-full">
          ...
          {/* ── Left: text ── */}
          <div className={`flex flex-col gap-3 md:gap-4 max-w-xs md:max-w-md lg:max-w-xl ${isUploadedBg ? "py-8" : "py-6"}`}>
            <span className={`text-[10px] md:text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full self-start ${isUploadedBg ? "bg-white/25 text-white" : "bg-emerald-100 text-emerald-700"}`}>
              {s.badge}
            </span>

            <h2
              style={{fontFamily:"'Cormorant Garamond',serif"}}
              className={`font-bold leading-tight whitespace-pre-line ${isUploadedBg ? "text-white drop-shadow-md" : s.textColor} text-2xl sm:text-3xl md:text-4xl lg:text-5xl`}
            >
              {s.title}
            </h2>

            <p className={`text-sm md:text-base leading-relaxed max-w-sm ${isUploadedBg ? "text-white/90 drop-shadow" : "text-slate-600"}`}>
              {s.text}
            </p>

            <button
              onClick={() => setPage(s.ctaAction)}
              className={`self-start font-bold px-6 py-2.5 md:px-8 md:py-3 rounded-full text-sm md:text-base transition-all active:scale-95 shadow-lg mt-1 ${
                s.ctaStyle === "blue"
          ...
          {/* ── Right: product image (only for non-uploaded slides) ── */}
          {!isUploadedBg && (
            <div className="hidden md:flex flex-1 justify-center items-center h-full py-4">
              <img
                src={s.img}
                alt={s.imgAlt}
                className="h-full max-h-72 lg:max-h-80 object-contain drop-shadow-2xl"
                draggable="false"
      ...
      <button
        onClick={prev}
        className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 md:w-10 md:h-10 bg-white/80 hover:bg-white rounded-full shadow-lg flex items-center justify-center text-slate-700 hover:text-emerald-700 transition-all active:scale-95 text-lg"
        aria-label="Previous"
      >‹</button>
      <button
        onClick={next}
        className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 md:w-10 md:h-10 bg-white/80 hover:bg-white rounded-full shadow-lg flex items-center justify-center text-slate-700 hover:text-emerald-700 transition-all active:scale-95 text-lg"
        aria-label="Next"
      >›</button>
      ...
      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
        {BANNER_SLIDES.map((_, i) => (
          <button
            ...
            className={`rounded-full transition-all ${i===current ? "bg-white w-6 h-2" : "bg-white/50 hover:bg-white/80 w-2 h-2"}`}
